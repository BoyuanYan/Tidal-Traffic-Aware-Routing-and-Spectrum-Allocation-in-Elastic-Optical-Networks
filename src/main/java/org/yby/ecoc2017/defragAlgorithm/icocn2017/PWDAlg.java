...
import org.yby.ecoc2017.defragAlgorithm.boyuan.EdgeWeightDescendComparator;
import org.yby.ecoc2017.defragAlgorithm.boyuan.MLSPDRAlg;
 ...
 * Predicted Weight Descending.根据机器学习预测结果，对业务进行权值降序排序，然后依次将目前正在使用该slot的业务选出来，
 * 一直到达到一定的比例γ。
 */
public class PWDAlg <E extends EonEdge> extends MLSPDRAlg<E> {

    int serviceShiftTime;
    int checkedServiceNum;

    public PWDAlg(SimpleWeightedGraph<EonVertex, E> graph, TreeMap<Integer, ServiceAssignment<E>> currentServices, Map<E, Double> occupiedSlotsNum) {
...
//        changeEdgeWeight();
        ...
        // 取其前num个进行重构。
        List<ServiceAssignment<E>> subOrderedSA = edgeWeightDescendList.subList(0, num);
