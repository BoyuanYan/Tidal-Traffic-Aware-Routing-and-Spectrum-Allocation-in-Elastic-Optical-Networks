 ...
 */
public class KPeriodicSPDRSA extends FirstFitRSAAlg {
    public KPeriodicSPDRSA(SimpleWeightedGraph<EonVertex, EonEdge> graph, ArrayList<Service> servicesQueue, ArrayList<Timestamp> servicesOrderedQueue) {
        ...
        for (int index=0; index<getServicesOrderedQueue().size(); index++) {
            ...
            if (timestamp.isStartTime()) {
                ...
                if (leaveServiceCount >= Configuration.threshold) {
                    // 如果离去业务超过阈值，则进行重构
                    KPeriodicSPDAlg<EonEdge> kperiodicSPDAlg = new KPeriodicSPDAlg<>(getGraph(), getCurrentServices());
                    // 把离去业务信息传入重构函数中。
                    kperiodicSPDAlg.defragment(serviceToBeAssigned);
                    Pair<Integer, Integer> pair = new Pair<>(kperiodicSPDAlg.getCheckedServiceNum(), kperiodicSPDAlg.getShiftTime());
