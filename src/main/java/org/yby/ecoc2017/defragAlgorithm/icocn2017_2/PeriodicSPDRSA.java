 ...
 */
public class PeriodicSPDRSA  extends FirstFitRSAAlg {
    public PeriodicSPDRSA(SimpleWeightedGraph<EonVertex, EonEdge> graph, ArrayList<Service> servicesQueue, ArrayList<Timestamp> servicesOrderedQueue) {
        ...
        for (int index=0; index<getServicesOrderedQueue().size(); index++) {
            ...
            if (timestamp.isStartTime()) {
                ...
                if (leaveServiceCount >= Configuration.threshold) {
                    // 如果离去业务超过阈值，则进行重构
                    PeriodicSPDAlg<EonEdge> periodicSPDAlg = new PeriodicSPDAlg<>(getGraph(), getCurrentServices());
                    // 把离去业务信息传入重构函数中。
                    periodicSPDAlg.defragment(serviceToBeAssigned);
                    Pair<Integer, Integer> pair = new Pair<>(periodicSPDAlg.getCheckedServiceNum(), periodicSPDAlg.getShiftTime());
